 ...
 *
 * Client component for the Admin Profile Settings form.
 *
 * Receives `role` from the parent Server Component (page.tsx) so that the
 * email field is conditionally editable for SUPER_ADMIN users.
 *
 * - SUPER_ADMIN: email is fully editable (sent in PUT body)
 * - LAWYER / other: email is read-only with helper text
 *
 * API calls:
 *   GET  /api/admin/profile  → populate form
 *   PUT  /api/admin/profile  → save changes
 */
...
export default function AdminSettingsForm({ role }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      // SUPER_ADMIN may update email; other roles omit it from the payload
        ...
        {status !== "loading" && (
          ...
          >
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              {/* Email — editable for SUPER_ADMIN, read-only for others */}
                ...
                {isSuperAdmin ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-text-muted">
                      Email cannot be changed here. Contact your system
                      administrator.
                    </p>
                  </>
                )}

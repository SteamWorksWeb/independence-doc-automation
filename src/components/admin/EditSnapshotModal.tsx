  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-3xl max-h-full bg-white rounded-xl shadow-2xl overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-400 [&::-webkit-scrollbar-thumb]:rounded-full">
        ...
        {/* ── Tab switcher ── */}
        <div className="flex border-b border-[#e5e7eb] bg-white shrink-0" role="tablist" aria-label="Borrower modal tabs">
          <button
            id="tab-borrower-data"
            role="tab"
            aria-selected={activeTab === "data"}
            aria-controls="tabpanel-borrower-data"
            onClick={() => setActiveTab("data")}
            className={[
              "px-6 py-3 text-[0.875rem] font-semibold border-b-2 transition-colors duration-150 cursor-pointer -mb-px",
              activeTab === "data"
          ...
          <button
            ...
            className={[
              "px-6 py-3 text-[0.875rem] font-semibold border-b-2 transition-colors duration-150 cursor-pointer -mb-px flex items-center gap-1.5",
              activeTab === "messages"
                ? "border-[#1d4ed8] text-[#1d4ed8]"
                : "border-transparent text-[#6b7280] hover:text-[#374151] hover:border-[#d1d5db]",
            ].join(" ")}
        ...
        {/* Borrower Data tab panel */}
        <div
          id="tabpanel-borrower-data"
          role="tabpanel"
        ...
        {activeTab === "messages" && (
          ...
          >
            {borrower.client?.id ? (
              <StaffMessageThread borrowerId={borrower.client.id} />
            ) : (
              <div className="flex flex-col items-center justify-center gap-3 py-16 px-8 text-center">
                <p className="text-sm text-[#6b7280]">
                  No client ID available for this borrower — cannot load messages.
                </p>
              </div>
            )}
          </div>
